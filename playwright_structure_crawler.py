...
def crawl_playwright(
    ...
    link_selector = crawl_cfg.get("link_selector") or crawl_cfg.get("article_link_selector") or "a[href]"
        ...
        try:
            with output.open("w", encoding="utf-8") as handle:
                while queue and len(visited) < max_pages:
                    ...
                    url = queue.pop(0)
                    if url in visited or not same_page_type(url, start_url, pattern):
                        continue
                    visited.add(url)
                    ...
                    try:
                        ...
                        try:
                            hrefs = page.locator(link_selector).evaluate_all(
                                "els => els.map(e => e.href || e.getAttribute('href')).filter(Boolean)"
                            )
                        except Exception:
                            hrefs = []
                        for href in hrefs:
                            full = urljoin(page.url, str(href)).split("#", 1)[0]
                            if full not in visited and same_page_type(full, start_url, pattern):
                                queue.append(full)
                    except Exception as exc:
                        record = {
                            "url": url,
